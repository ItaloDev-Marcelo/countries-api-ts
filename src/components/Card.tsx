...
const Card = ({name, population,region,capital,img,id,handleDatails, toggle}:CardProps) => {
  ...
  return (
   <div onClick={() =>  handleDatails(id)} className={`card cursor-pointer  w-[97%] md:w-[370px] tb:w-[445px] 
    xl:w-[275px]  m-2 shadow  transition duration-500 ease-in-out ${toggle ? 'bg-Blue-900 text-white ' : 'bg-Grey-50 text-Grey-950'}`} key={id}>
  <figure>
    <img
      src={img}
      alt={name} 
